...
local adminStuffs = {
	[Warden.ADMIN_NET.ADMIN_LEVEL] = function(ply)
		if not ply:WardenGetCmdPerm("warden_admin_level") then return end
	...
	[Warden.ADMIN_NET.CLEAR_DISCONNECTED] = function(ply)
		if not ply:WardenGetCmdPerm("warden_cleanup_disconnected") then return end
	...
	[Warden.ADMIN_NET.CLEAR_ENTS] = function(ply)
		if not ply:WardenGetCmdPerm("warden_cleanup_entities") then return end
	...
	[Warden.ADMIN_NET.FREEZE_ENTS] = function(ply)
		if not ply:WardenGetCmdPerm("warden_freeze_entities") then return end
